...
class RealGraph : public std::enable_shared_from_this<RealGraph> {
  public:
    ...
    int Update(const std::shared_ptr<RealGraph> &update_graph);
    void DynamicAdd(const std::shared_ptr<RealNode> &module_node,
                    const bmf_sdk::JsonParam &inputs,
                    const bmf_sdk::JsonParam &outputs);
    void DynamicRemove(const bmf_sdk::JsonParam &node_config);
...
class BMF_ENGINE_API Graph {
  ...
  public:
    ...
    /**
     * Apply a graph generated by DynamicAdd(), DynamicRemove(), or
     * DynamicReset() to this running graph.
     */
    int Update(const Graph &update_graph);

    /**
     * Mark the nodes in this graph for dynamic insertion. The optional inputs
     * and outputs objects use the form {"alias": "node", "streams": 2}.
     */
    void DynamicAdd(
        const Node &module_node,
        const bmf_sdk::JsonParam &inputs = bmf_sdk::JsonParam(),
        const bmf_sdk::JsonParam &outputs = bmf_sdk::JsonParam());

    /** Build an update graph that removes the node identified by alias. */
    void DynamicRemove(const bmf_sdk::JsonParam &node_config);

    /** Build an update graph that resets the node identified by alias. */
    void DynamicReset(const bmf_sdk::JsonParam &node_config);
